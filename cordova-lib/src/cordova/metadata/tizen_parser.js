...
function tizen_parser(project) {

    // Call the base class constructor
    Parser.call(this, 'tizen', project);

    this.path = project;
    this.config_path = path.join(project, 'config.xml');
    this.manifest_path = path.join(this.www_dir(), 'config.xml');
}

require('util').inherits(tizen_parser, Parser);

module.exports = tizen_parser;

// Returns a promise.
tizen_parser.prototype.update_from_config = function(config) {
    ...
    var icons = config.getIcons('tizen');
...
tizen_parser.prototype.www_dir = function() {
    return path.join(this.path, 'www');
};

// Used for creating platform_www in projects created by older versions.
tizen_parser.prototype.cordovajs_path = function(libDir) {
    var jsPath = path.join(libDir, 'cordova-lib', 'cordova.js');
    return path.resolve(jsPath);
};

tizen_parser.prototype.cordovajs_src_path = function(libDir) {
    var jsPath = path.join(libDir, 'cordova-js-src');
    return path.resolve(jsPath);
};

// Replace the www dir with contents of platform_www and app www.
tizen_parser.prototype.update_www = function() {
...
tizen_parser.prototype.update_overrides = function() {
    var projectRoot = util.isCordova(this.path);
    var mergesPath = path.join(util.appDir(projectRoot), 'merges', 'tizen');
...
tizen_parser.prototype.config_xml = function(){
    return this.config_path;
};

// Returns a promise.
tizen_parser.prototype.update_project = function(cfg) {
